...
export default function Druh() {
    ...
    const DeleteButton = (tableRow) =>
        <button className="btn btn-danger mx-2"
                onClick={() => deleteEntityId(tableRow.original.druhId)}
        >
            Odstranit
        </button>;
    ...
    const ViewLink = (tableRow) =>
        <Link className="btn btn-primary mx-2"
              to={Constants.guiViewPath(tableRow.original.druhId)}
        >
            Zobrazit
        </Link>;
    ...
    const EditLink = (tableRow) =>
        <Link className="btn btn-outline-primary mx-2"
              to={Constants.guiEditPath(tableRow.original.druhId)}
        >
            Upravit
        </Link>
