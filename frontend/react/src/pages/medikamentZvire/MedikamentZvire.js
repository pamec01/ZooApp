...
export default function MedikamentZvire() {
    ...
    const DeleteButton = (tableRow) =>
        <button className="btn btn-danger mx-2"
                onClick={() => deleteEntityId(tableRow.original.medikamentZvireId)}
        >
            Odstranit
        </button>;
    ...
    const EditLink = (tableRow) =>
        <Link className="btn btn-outline-primary mx-2"
              to={Constants.guiEditPath(tableRow.original.medikamentZvireId)}
        >
            Upravit
        </Link>
