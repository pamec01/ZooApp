...
export default function Osetrovatel() {
    ...
    const DeleteButton = (tableRow) =>
        <button className="btn btn-danger mx-2"
                onClick={() => deleteEntityId(tableRow.original.osetrovatelId)}
        >
            Odstranit
        </button>;
    ...
    const ViewLink = (tableRow) =>
        <Link className="btn btn-primary mx-2"
              to={Constants.guiViewPath(tableRow.original.osetrovatelId)}
        >
            Zobrazit
        </Link>;
    ...
    const EditLink = (tableRow) =>
        <Link className="btn btn-outline-primary mx-2"
              to={Constants.guiEditPath(tableRow.original.osetrovatelId)}
        >
            Upravit
        </Link>
