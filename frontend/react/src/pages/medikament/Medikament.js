...
export default function Medikament() {
    ...
    const DeleteButton = (tableRow) =>
        <button className="btn btn-danger mx-2"
                onClick={() => deleteEntityId(tableRow.original.medikamentId)}
        >
            Odstranit
        </button>;
    ...
    const ViewLink = (tableRow) =>
        <Link className="btn btn-primary mx-2"
              to={Constants.guiViewPath(tableRow.original.medikamentId)}
        >
            Zobrazit
        </Link>;
    ...
    const EditLink = (tableRow) =>
        <Link className="btn btn-outline-primary mx-2"
              to={Constants.guiEditPath(tableRow.original.medikamentId)}
        >
            Upravit
        </Link>
